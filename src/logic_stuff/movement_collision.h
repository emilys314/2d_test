...
void processMovementCollisions(Timer timer, Inputs &inputs, Entity_Manager &entity_manager, int player_id) {
    ...
    glm::vec2 new_position = entity_manager.getSquare(player_id).position + entity_manager.getMovement(player_id).velocity;
    ...
    entity_manager.getSquare(player_id).position += entity_manager.getMovement(player_id).velocity;
}
